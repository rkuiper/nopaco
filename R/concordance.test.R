...
dwish<-function (W, v, S,log.p=FALSE) 
{
    ##This function has been obtained from the MCMCpack packages but added the log.p argument
    if (!is.matrix(S)) 
        S <- matrix(S)
    if (nrow(S) != ncol(S)) {
    ...
    if (!log.p){
        ...
        gammapart<-sum(lgamma((v + 1 - c(1:k))/2))
        denom <- gammapart + log(2)*(v * k/2)+log(pi)*(k * (k - 1)/4)
     
        if (detS<=0 ){ 
            termA<-log(detS^(-v/2))
        } else {termA<-log(detS)*(-v/2)}
        if (detW<=0 ){ 
            termB<-log( detW^((v - k - 1)/2))
        } else {termB<-log(detW)*((v - k - 1)/2)}
        hold <- solve(S) %*% W
        tracehold <- sum(hold[row(hold) == col(hold)])
        num <- termA +termB+(-1/2 * tracehold)
...
setMethod("initialize",
    ...
    definition = function(.Object, ...){
        ...
        if (dotList[["psi1"]]<0.4 | dotList[["psi1"]]>1  ) {
            stop("psi1 must be 0.4<psi1<=1")
        }
        if (!is.numeric(dotList[["psi2"]]) | length(dotList[["psi2"]])!=1 ){
            stop("psi2 must be a numeric value of length 1")
        }
        if (!is.na(dotList[["psi2"]])){
            if (dotList[["psi2"]]<0.4 | dotList[["psi2"]]>1  ) {
                stop("psi2 must be 0.4<psi2<=1")
            }
        }
    ...
    if (is.null(y)){
        if (method=="exact"){
            ...
            if (!inherits(exactDist,"try-error")){
                ...
                #    ci.lower<-min(exactDist[,1])
                #} else if (alternative=='two.sided'){
                #    p<- sum(exactDist[ ! ((exactDist[,1] > 2/3-abs(obs-2/3)) &  (exactDist[,1] < 2/3+abs(obs-2/3))) ,2])
        ...
        if (method != "exact"){        
            ...
            if (method=="altBeta"){
                ...
                #if (alternative=='less') {
                #    p<-pbeta(obs-x-iii,shape1=alphaPar,shape2=betaPar,lower.tail=TRUE)
                #    est<-optimize(interval=c(1,1e8),mu=obs-x-iii,p=p,lower.tail=FALSE,targetValue=2/3-x-iii, f=.confEstimatorBeta)
                #    betaEst<-est$minimum
                #    alphaEst<-(obs-x-iii)*betaEst/(1-(obs-x-iii))
                #    ci.lower=.minPsi(bn)
                #    ci.upper=qbeta(alpha,alphaEst,betaEst,lower.tail=FALSE)+x+iii
                #} else if (alternative=='greater') {
                    ...
                    ci.upper=1
                #} else if (alternative=='two.sided') {
                #    p.left<-pbeta(alphaPar/(alphaPar+betaPar)-(abs((obs-x-iii)-alphaPar/(alphaPar+betaPar))) ,shape1=alphaPar,shape2=betaPar,lower.tail=TRUE)
                #    p.right<-pbeta(alphaPar/(alphaPar+betaPar)+(abs((obs-x-iii)-alphaPar/(alphaPar+betaPar))) ,shape1=alphaPar,shape2=betaPar,lower.tail=F)
                #    p<-p.left+ p.right
                #    if (obs<(2/3)){
                #        est<-optimize(interval=c(1,1e8),mu=obs-x-iii,p=p,lower.tail=FALSE,targetValue=2/3-x-iii, f=.confEstimatorBeta)
                #        betaEst<-est$minimum
                ...
                #if (alternative=='less') {
                #    p<-pbeta(obs,shape1=alphaPar,shape2=betaPar,lower.tail=TRUE)
                #    est<-optimize(interval=c(1,1e8),mu=obs,p=p,lower.tail=FALSE,targetValue=2/3,f=.confEstimatorBeta)
                #    betaEst<-est$minimum
                #    alphaEst<-obs*betaEst/(1-obs)
                #    ci.lower=.minPsi(bn)
                #    ci.upper=qbeta(alpha,alphaEst,betaEst,lower.tail=FALSE)
                #} else if (alternative=='greater') {
                ...
                #} else if (alternative=='two.sided') {
                #    p.left<-pbeta(alphaPar/(alphaPar+betaPar)-(abs(obs-alphaPar/(alphaPar+betaPar))) ,shape1=alphaPar,shape2=betaPar,lower.tail=TRUE)
                #    p.right<-pbeta(alphaPar/(alphaPar+betaPar)+(abs(obs-alphaPar/(alphaPar+betaPar))) ,shape1=alphaPar,shape2=betaPar,lower.tail=F)
                #    p<-p.left+ p.right
                #           
                #    if (obs<(2/3)){
                #        est<-optimize(interval=c(1,1e8),mu=obs,p=p,lower.tail=FALSE,targetValue=2/3, f=.confEstimatorBeta)
                ...
                #if (alternative=='less') {
                #    p<-pnorm(q=obs,mean=2/3,sd=sqrt(obsvar),lower.tail=TRUE)
                #    if (p>(alpha/10)) { ##otherwise perform estimate by simulation (see below)
                #        ci.lower<-.minPsi(bn)
                #        ci.upper<-qnorm(alpha,mean=obs,sd=sqrt(obsvar),lower.tail=F)
                #    }
                #} else if (alternative=='greater') {
                ...
                #} else if (alternative=='two.sided') {
                #       p<-2*pnorm(q=abs(obs-2/3)+2/3,mean=2/3,sd=sqrt(obsvar),lower.tail=F)
                #    if (p>(alpha/10)) { ##otherwise perform estimate by simulation (see below)
                #       ci.lower<-qnorm(alpha/2,mean=obs,sd=sqrt(obsvar),lower.tail=T)
                #           ci.upper<-qnorm(alpha/2,mean=obs,sd=sqrt(obsvar),lower.tail=F)
                #    }
                #}
        ...
        cormat<-am$mat
        cormat[]<-rfromPsi(am$mat)
        myf2<-function(par,cormat,nMat){
            #cat(log(par),"\n")
            cormat[cormat==1]<-1-1e-6 ##Aim: the non-diagonal elements should not be 1
            diag(cormat)<-1
            r1<-par[1]
            r2<-par[2]
            S1<-matrix(c(1,r1,r1,1),2,2)
            S2<-matrix(c(1,r2,r2,1),2,2)
            res<-nMat*NA
            for (i in c(1:(-1+ncol(nMat)))){
            for (j in c((i+1):(ncol(nMat)))){
                WW<-matrix(c(1,cormat[i,j],cormat[j,i],1),2,2)*(nMat[i,j])
                if ( (i>(ncol(nMat)/2)& j>(ncol(nMat)/2)) | (i<=(ncol(nMat)/2)& j<=(ncol(nMat)/2)) ){
                    ##Ondiag quad.
                    res[i,j]<-dwish(W=WW,v=nMat[i,j]-1,S=S1,log.p=TRUE)
                } else {
                    ##Offdiag quad.
                    res[i,j]<-dwish(W=WW,v=nMat[i,j]-1,S=S2,log.p=TRUE)
                }
            }}
        ...
        b<-ncol(x)
        r1<-abs((sum(cormat[1:b,1:b]+cormat[-c(1:b),-c(1:b)])/2 - b)/(b*b-b)) #Average observed correlation
        r2<-(sum(cormat[1:b,-c(1:b)]+cormat[-c(1:b),1:b])/2)/(b*b) #Average observed correlation
        par<-c(r1,r2)
        #par<-optim(par=c(r1,r2),fn=myf2,cormat=cormat,nMat=am$n,upper=c(1-1e-6,1-1e-6),lower=c(-1+1e-6,-1+1e-6),method='L-BFGS-B')$par 
