...
\begin{figure}[H]
\centering
<<modelA, fig=TRUE,echo=FALSE,width=4,height=4>>=
scatterBarNorm(scores[["modelA"]],pch=21,bg='black',main="model A")
@
<<modelB, fig=TRUE,echo=FALSE,width=4,height=4>>=
scatterBarNorm(scores[["modelB"]],pch=21,bg='black',main="model B")
@
\caption{The scores for replicates 1 (horizontally) versus replicates 2 (vertically) as obtained by model A (top) and model B (bottom). The marginal distributions are given by histograms.\label{fig:dataPlot}}
\end{figure}
...
The test shows that the two models differ significantly in concordance. In order to extract the results we can use the following commands:
<<>>=
coef(diffTest)
diffTest$pvalue
diffTest$psi
...
\begin{center}
	...
	0&0&0.2
	\end{bmatrix}$
\end{center}, resulting in measurement errors in the second replicate that are expected to increase for genes 1 to 3.
